...
$utilisateurstotalsreq=$connect->query('SELECT id FROM utilisateur');
$utilisateurstotals=$utilisateurstotalsreq->rowCount();
$pagestotales=ceil($utilisateurstotals/$utilisateursparpage);
...
$depart= ($pagecourante-1)*$utilisateursparpage;
...
$req=$connect->prepare('SELECT * FROM utilisateur ORDER BY Nom ASC LIMIT '.$depart.','.$utilisateursparpage);
$req->execute();
$count = $req->rowCount();
...
<html>
<head>
	<title>Suppression_employé</title>
	<meta charset="utf-8">
...
<form method="POST" action="" accept-charset="utf-8">
	<div id="connex"><div class="img"></div><span class="out">Login: <?=$userinfo['login'];?></span><br><br><span class="out">Type: <?= $_SESSION['habilitation']; ?></span><input type="submit" name="acceuil" value="Acceuil" class="in"></input><input type="submit" name="deconnexion" value="Se déconnecter" class="in"></input><input type="submit" name="modifier" value="Modifier mon profil"><div id='ajout'><input type="submit" name="ajouter" value="Ajouter un élève" id="in"></div><?php if($getpriv==$admin AND $getpriv==$_SESSION['habilitation']){ ?><input type="submit" name="inscrire" value="Ajouter un employé" class="in"></input><input type="submit" name="supprimer" value="Supprimer un employé" class="in" disabled></input></span><?php }elseif($getpriv==$uti AND $getpriv==$_SESSION['habilitation']){ ?>
	...
	<div class="entete"><div id="rev">Version 1.0</div><div class="titre">TOUS EN MUSIQUE</div>
		<div id="st">Fiche de supression des employés</div>
		...
		<div id="dt">
			...
			<div id="tt">Liste des employés</div>
			<div id="result">
			...
			if($count > 0){
				while ($result = $req->fetch(PDO::FETCH_OBJ)) {
					if($result->id==$_SESSION['id']){ 
							echo "<tr>
									<td align='center'>
										<img src='employes/avatars/".$result->avatar."' style='border-radius: 50%;width:50px;height:50px;'/>
									</td>
									<td align='center'>
										".$result->nom."
									</td>
							...
							echo "<tr>
									...
									<td align='center'>
										<a href='supprimer.php?id=".$_SESSION['id']."&idu=".$result->id."&log=".$result->login."'><input type='button' value='Supprimer'></a>
									</td>
								</tr>";
					}		 		
				}		 		
			}else{
				echo "<div style='text-align:center'>Aucun utilisateurs inscrits</div>";
			} 
			?>
			</table>
			<?php 
			//affichage des pages de la pagination
				for($i=1;$i<=$pagestotales;$i++){
					if ($pagecourante==$i) {
						...
						echo "<a href='supression_utilisateur.php?id=".$_SESSION['id']."&page=".$i."&pv=".$_SESSION['habilitation']."'>".$i." </a> ";
					}
				}
			?>
			</div>
			<?php
				if(isset($_GET['msg']) AND isset($_GET['log'])){
					if($_GET['msg']==1){
						echo "<script type='text/javascript'>alert('L\'utilisateur ".$_GET['log']." a bien été supprimer');</script>";
					}else{
						echo "<script type='text/javascript'>alert('L\'utilisateur ".$_GET['log']." n'a pas pu être supprimer pour des raisons inconnues');</script>";
					}
				}
